= image:../logo.svg[Boat Spring Boot] Boat Spring Boot:  基于 Spring Boot 和  {boat-url}[Boat] 的 SrcLab 核心库
:toc:
:toclevels: 3
:toc-title: 目录
:last-update-label!:
...
* `WebExceptionResponseHandler`: 为 `WebExceptionService` 服务的Web异常处理器;
