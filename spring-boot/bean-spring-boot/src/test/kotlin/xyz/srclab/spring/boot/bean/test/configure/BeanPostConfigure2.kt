...
@Component
open class BeanPostConfigure2 : BeanPostConfigure {
    ...
    @PostConstruct
    private fun init() {
        println("BeanPostConfigure2.init")
    ...
    override fun configureAfterInitialization(context: BeanPostContext): Any {
        println("BeanPostConfigure2.configureAfterInitialization, context=${context.name}")
        return context.bean
    }

    override fun configureBeforeInitialization(context: BeanPostContext): Any {
        println("BeanPostConfigure2.configureBeforeInitialization, context=${context.name}")
        return context.bean
    }
}